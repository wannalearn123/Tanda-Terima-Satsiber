...
final class Pengiriman
{
    public static function create(PDO $pdo, array $d): int
    {
        ...
        try {
            $st = $pdo->prepare(
                ...
                (nomor_referensi, pusat_penerima_id, nama_penerima, pangkat_golongan, jabatan,
                 tanggal, pukul, telp_hp, tanda_tangan_path, created_at, updated_at)
                VALUES
                (:nomor, :pusat, :nama, :pangkat, :jabatan, :tanggal, :pukul, :telp, :ttd, :now, :now)"
            );
            $st->execute([
                ':nomor' => $d['nomor_referensi'],
                ':pusat' => $d['pusat_penerima_id'],
        ...
        try {
            $st = $pdo->prepare(
                "UPDATE pengiriman SET
                    nomor_referensi = :nomor, pusat_penerima_id = :pusat, nama_penerima = :nama,
    ...
    {
        $st = $pdo->prepare(
            "SELECT p.*, c.nama AS pusat_nama, c.kode AS pusat_kode
             FROM pengiriman p LEFT JOIN pusat_penerima c ON c.id = p.pusat_penerima_id
